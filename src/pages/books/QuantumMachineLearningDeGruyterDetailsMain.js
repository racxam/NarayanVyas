...
const AIDrivenAlzheimerIGIDetailsMain = () => {
  const bookDetails = booksData.find(book => book.id === 'quantum-machine-learning-quantum-algorithms-neural-networks');
  if (!bookDetails) {
    return <div>Book details not found.</div>;
  }

  const relatedBooks = getRelatedBooks('quantum-machine-learning-quantum-algorithms-neural-networks');
