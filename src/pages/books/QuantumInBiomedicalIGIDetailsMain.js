...
const AIDrivenAlzheimerIGIDetailsMain = () => {
  const bookDetails = booksData.find(book => book.id === 'quantum-innovations-nexus-biomedical-intelligence');
  if (!bookDetails) {
    return <div>Book details not found.</div>;
  }

  const relatedBooks = getRelatedBooks('quantum-innovations-nexus-biomedical-intelligence');
