...
const AIDrivenAlzheimerIGIDetailsMain = () => {
  const bookDetails = booksData.find(book => book.id === 'ai-driven-alzheimer-disease-detection');
  if (!bookDetails) {
    return <div>Book details not found.</div>;
  }

  const relatedBooks = getRelatedBooks('ai-driven-alzheimer-disease-detection');
