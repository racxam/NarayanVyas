...
const AIDrivenAlzheimerIGIDetailsMain = () => {
  const bookDetails = booksData.find(book => book.id === 'innovations-in-ml-iot-for-water-management');
  if (!bookDetails) {
    return <div>Book details not found.</div>;
  }

  const relatedBooks = getRelatedBooks('innovations-in-ml-iot-for-water-management');
